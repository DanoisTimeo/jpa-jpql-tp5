...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParAnneeNaissance() {
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a WHERE YEAR(a.anniversaire) = :anniversaire", Acteur.class);
		query.setParameter("anniversaire", "1985");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r WHERE r.nom = :role", Acteur.class);
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f WHERE f.annee = :anneeFilm", Acteur.class);
		query.setParameter("anneeFilm", "2015");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParPays() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p WHERE p.nom = :paysOrgine", Acteur.class);
		query.setParameter("paysOrgine", "France");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.pays p WHERE f.annee = :anneeFilm AND p.nom = :paysOrgine", Acteur.class);
		query.setParameter("anneeFilm", "2017");
		query.setParameter("paysOrgine", "France");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles ro JOIN ro.film f JOIN f.realisateurs rea WHERE rea.identite = :identiteRealisateur AND f.annee BETWEEN :dateDebut AND :dateFin", Acteur.class);
		query.setParameter("identiteRealisateur", "Ridley Scott");
		query.setParameter("dateDebut", "2010");
		query.setParameter("dateFin", "2020");
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireRealisateursParActeur() {
		TypedQuery<Realisateur> query = em.createQuery("SELECT DISTINCT rea FROM Realisateur rea JOIN rea.films f JOIN f.roles ro JOIN ro.acteur a WHERE a.identite = :identiteActeur", Realisateur.class);
		query.setParameter("identiteActeur", "Brad Pitt");
